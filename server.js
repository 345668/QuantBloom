...
// Technical indicators live in bot/indicators.js so the live route and the
// backtester compute them with identical code — see that file for why.
...
app.get('/api/v1/technical', async (req, res) => {
  try {
    ...
    // Same computeTechnical the backtester calls — see bot/indicators.js.
...
// ---------------------------------------------------------------------------
// GET /api/v1/bot/backtest?symbol=SPY&range=5y&mode=single|walkforward|sweep
// Runs the SAME strategies and indicators the live engine uses.
// ---------------------------------------------------------------------------
app.get('/api/v1/bot/backtest', async (req, res) => {
  try {
    ...
    const strategies = req.query.strategies
      ? String(req.query.strategies).split(',').filter(Boolean) : undefined;
    ...
    const result = mode === 'walkforward' ? walkForward(candles, { ...opts, folds: 4 })
      : mode === 'sweep' ? sweepStrategies(candles, opts)
      : runBacktest(candles, opts);

    // Backtests are deterministic for a given history; cache hard.
    cacheSet(cacheKey, { mode, symbol, range, ...result }, 3600000);
